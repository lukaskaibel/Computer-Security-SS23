...
def encrypt_bully(path_to_file_directory, number_of_keys):
    hashkeys = read_from_file(path_to_file_directory, number_of_keys, "key")
    plaintext_messages = read_from_file(path_to_file_directory, number_of_keys, "Ciphertext-dirty")

    encrypted_messages = OTP(hashkeys, plaintext_messages)

    # Comment out shuffling
    # encrypted_messages, hashkeys = zip(*shuffle(encrypted_messages, hashkeys))

    write_to_file(path_to_file_directory, encrypted_messages, "Encrypted-Ciphertext-dirty")
    # Keys also don't have to be written out
    #write_to_file(path_to_file_directory, hashkeys, "Key")
...
if __name__ == "__main__":
    try:
        ...
        elif operation_mode == "encrypt_bully":
            bully_keys = 13
            encrypt_bully(path_to_file_directory, bully_keys)
        ...
        raise SystemExit(f"Usage: {sys.argv[0]} <encrypt|decrypt|encrypt_bully> <path_to_file_directory>")
